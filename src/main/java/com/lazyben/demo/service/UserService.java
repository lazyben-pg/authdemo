...
public class UserService {
    ...
    public Map<String, String> login(String username, String password) {
        ...
        if (authenticate == null) throw new AuthenticationException("账号或密码错误！");

        var userDetails = (UserDetails) authenticate.getPrincipal();

        //用户鉴权
        SecurityContextHolder.getContext().setAuthentication(authenticate);
        String token = RedisUtils.get(TOKEN_KEY + userDetails.getUsername()) == null ? "" : RedisUtils.get(TOKEN_KEY + userDetails.getUsername());
